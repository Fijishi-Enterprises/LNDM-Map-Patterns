...
vignette: >
  %\VignetteIndexEntry{Differences FRAGSTATS|landscapemetrics|SDMTools}
...
**The comparison is currently outdated because the example landscape slightly changed and we trying to re-calculate the FRAGSTATS results currently. Additionally, we cant install `SDMTools` anymore (R > 4.0).**
...
While the results are identical for single patches and the mean of all patches, there are some slight differences between **lanscapemetrics** and FRAGSTATS for the standard deviation and the coefficient of variation.
...
# convert to terra
ras <- terra::rast(mat, xmin = 0, xmax = 13, ymin = 0, ymax = 13)
